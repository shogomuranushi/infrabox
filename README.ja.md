...
| 📜 | 初回起動時のセットアップスクリプト（`ib setup-script`） |
| 🔄 | ローカルファイルを新規 VM に自動転送（`ib sync`） |
| 🔗 | Claude Code SSH 連携（`ib ssh-proxy`） |
| 📦 | `ib` CLI ツール |
...
`ib ssh-proxy` は stdin/stdout 上で SSH サーバーとして動作し、WebSocket exec エンドポイントへブリッジします。Claude Code デスクトップアプリのような、本物の SSH プロトコルを必要とするツールから VM に直接接続できます。

```
Claude Code ──[SSH プロトコル]──▶ ib ssh-proxy ──[WebSocket]──▶ VM pod
```
...
その後、Claude Code の SSH 接続設定で `ubuntu@infrabox-<VM名>` を指定するだけで接続できます。
...
# セットアップスクリプト: 初回 VM 起動時にシェルスクリプトを自動実行
ib setup-script set ./setup.sh  # スクリプトを登録
ib setup-script show            # 登録中のスクリプトを表示
...
#     User ubuntu
#     ProxyCommand ib ssh-proxy %h
# Claude Code から ubuntu@infrabox-<VM名> で接続可能
ib ssh-proxy <name>        # ProxyCommand ブリッジ（SSH により自動起動）
```
...
| `GET` | `/v1/vms/{name}/exec` | WebSocket シェルセッション（tmux） |
| `GET` | `/v1/vms/{name}/exec-command?cmd=` | WebSocket 任意コマンド実行（tmux なし） |
| `POST` | `/v1/vms/{name}/run` | シェルコマンドを実行し stdout+stderr を返す |
