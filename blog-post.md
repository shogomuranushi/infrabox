...
[exe.dev](https://exe.dev) というサービスを知ったのは半年くらい前だ。

コマンド一発でLinux VMが起動して、HTTPS URLが自動で払い出される。SSHキーの管理もない。ポート公開の設定もない。ただ動かすだけ。月$20でVM25台まで使えて、Claude CodeやCodexもそのまま動く。

「これ、社内にあったら最高では」と思った。

ただ、SaaS に全部預けるのは少し引っかかった。社内のコードや API キーを外部サービスのマシンに置くのは、セキュリティポリシー的にも運用的にも整理が必要になる。それならいっそ自前で作ってしまおう、というのが InfraBox の出発点だ。
...
背景には AI コーディングの普及がある。

Claude Code のようなエージェントを使っていると、「ちょっとこの PoC を誰かに見せたい」という瞬間が増える。でもその瞬間、現実の壁にぶつかる。Cloud Run に載せるか？サーバーレスに書き直さないと動かないかも。Cloud SQL は高い、IAM 設定が面倒...。

本来やりたいことは「動くものを今すぐ見せる」だけなのに、気づいたらインフラの意思決定をしている。

InfraBox はその面倒を丸ごとなくす。`ib create` したら 10 秒以内に Linux 環境が立ち上がり、URL が発行される。あとは好きに使えばいい。
